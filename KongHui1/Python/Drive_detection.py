...
def check_networkdrivers():
    hardware_types = {
        "network": "网络适配器驱动"
    }

    wmic_queries = {
        "network": ["powershell", "-Command",
                    "Get-WmiObject -Class Win32_NetworkAdapterConfiguration | Where-Object { $_.IPEnabled -eq $true } | Select-Object -ExpandProperty Description"]
    }

    for hardware, name in hardware_types.items():
        try:
            result = subprocess.run(wmic_queries[hardware], capture_output=True, text=True, check=True)
